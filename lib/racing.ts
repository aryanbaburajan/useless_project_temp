...
export function validateRun(run:Run,gateIds:string[]){if(!run.displayName.trim()||run.displayName.length>32||!Number.isFinite(run.elapsedMs)||run.elapsedMs<10_000||run.elapsedMs>7_200_000)return "Invalid elapsed time or name.";if(!Number.isFinite(run.accuracyM)||run.accuracyM>75||run.accuracyM<0)return "GPS accuracy is too poor.";if(!Number.isFinite(run.maxSpeedMps)||run.maxSpeedMps>85||run.maxSpeedMps<0)return "Speed metric is implausible.";if(run.gateSequence.join("|")!==gateIds.join("|"))return "The required gates were not crossed in order.";return null}
